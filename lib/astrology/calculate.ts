...
//
// Sources: Swiss Ephemeris algorithms, Meeus "Astronomical Algorithms",
// standard LST/GST conversion formulas.
//
// Note: Moon position uses a simplified algorithm (accuracy ~1-2°).
// For production-grade accuracy, integrate Swiss Ephemeris or Skyfield.
...
export function calculateChart(birth: BirthData): AstroChart {
  const hourUT = (birth.hour ?? 12) + (birth.minute ?? 0) / 60;
  const jd = toJulianDate(birth.year, birth.month, birth.day, hourUT);
